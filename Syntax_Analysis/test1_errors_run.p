...
{Scope 1 start}
   function 2 _try_me(i,j:integer): real;
   {Scope 2 start}
   const omega='W'; seira = -g*.314159E1;
   begin
   if (i+j > omega) then {scope 3 start} _try_me :=0.0 {scope 3 end}
   else
      {scope 3 start}
      if (i > omega) then {scope 4 start} _try_me :=-1.0 {scope 4 end}
      else {scope 4 start} _try_me := 1.0 {scope 4 end}
      {scope 3 end}
   end;
   {scope 2 end}
begin
